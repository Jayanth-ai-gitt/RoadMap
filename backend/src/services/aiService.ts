...
// Simple interface for OpenAI responses
...
export const aiService = {
   ...
   */
  async generateRoadmap(desiredCareer: string, experienceLevel: string, currentSkills: string) {
...
Please customize the roadmap. Omit or compress skills they already have. Give them pragmatic, production-focused guidance for a junior to work effectively in a team.`;
   ...
   */
  async generateStudyPlan(userId: string, activeRoadmap: any, studyHours: number) {
...
Skills to learn: ${JSON.stringify(skillsToPlan.slice(0, 3).map((s: any) => ({ name: s.name, description: s.description })))}
...
Please distribute these topics into a realistic 5-day week study plan. Emphasize senior engineer suggestions.`;
   ...
   */
  async analyzeResume(resumeText: string, careerName: string, requiredSkills: string[], requiredProjects: string[], requiredCerts: string[]) {
...
Required Certifications: ${JSON.stringify(requiredCerts)}`;
    ...
    try {
      ...
      projectKeywords.forEach(kw => {
        if (resumeLower.includes(kw)) projectKeywordCount++;
      });
      ...
      if (projectKeywordCount < 4) score -= 15;
   ...
   */
  async generateChatResponse(message: string, userProfile: any, progress: any) {
